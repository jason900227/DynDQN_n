...
for n_steps_cycle in n_steps_cycles:
    ...
    for i, n_step in enumerate(n_steps):
        file_path = f"./results/{env}/Grid_search/DynDQN_n_E/Reward/Reward_DynDQN_n{n_step}_E{n_steps_cycle}.csv"
        ...
        plt.plot(data['Epoch number'], data['Smoothed Episode reward'], label=f'DynDQN_n{n_step}_E{n_steps_cycle}', linestyle='-', color=colors[i], linewidth=2)
    ...
    # Save the plot
    save_path = f"./results/{env}/Grid_search/DynDQN_n_E/n_steps_freq_{n_steps_cycle}.png"
