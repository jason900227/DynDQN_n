...
env = "Acrobot-v1"
n_steps_cycles = [1000, 1200, 1400, 1600]
...
for n_steps_cycle in n_steps_cycles:
    ...
    for i, n_step in enumerate(n_steps):
        ...
        print(f"DynDQN_n{n_step}_T{n_steps_cycle} Last 15 epoch average reward std: {data['Smoothed Episode reward std'].tail(15).mean()}\n")
            
        # Plot the smoothed data
        ax.plot(data['Epoch number'], data['Smoothed Episode reward'], label=f'DynDQN_n{n_step}_T{n_steps_cycle}', linestyle='-', color=colors[i], linewidth=2)
