...
class Runner:
    def __init__(self, args, env_name, seed):
        ...
        self.algorithm = f'DynDQN_n{self.n_steps}_T{self.args.n_steps_cycle}'
    ...
    def run(self):
        ...
        for t in progress_bar:
            ...
            if done:
                ...
                mean_cv_Q_value = np.array(cv_Q_value_list[-25:]).mean()
                std_cv_Q_value = np.std(np.array(cv_Q_value_list[-25:]))
            ...
            if (t + 1) % self.args.n_steps_cycle == 0 and len(cv_Q_value_list) != 0:
                ...
                current_threshold = current_threshold * 0.97
...
if __name__ == '__main__':
    ...
    for n_steps in n_steps_list:
        for n_steps_cycle in n_steps_cycle_list:
            ...
            parser.add_argument("--n_steps_cycle", type=int, default=n_steps_cycle, help='Number of time-steps for update n steps')
