...
class Runner:
    def __init__(self, args, env_name, seed):
        ...
        self.algorithm = 'DQN_LNSS_N' + str(self.N_steps)
    ...
    def run(self):
        ...
        for t in progress_bar:
            ...
            exp_buffer.append((state, action, reward, next_state, terminal, done))

            # Store data in replay buffer
            if len(exp_buffer) >= self.N_steps:
                initial_state, initial_action, discounts_accumulate_reward, end_state, end_terminal, end_done = exp_buffer.popleft()
                gamma = self.args.gamma

                for (_, _, r, _, _, _) in exp_buffer:
                    discounts_accumulate_reward += r * gamma
                    gamma *= self.args.gamma

                discount_factor = (self.args.gamma - 1)/(gamma - 1)
                discounts_accumulate_reward = discount_factor * discounts_accumulate_reward
                self.replay_buffer.store_transition(initial_state.cpu().numpy(), initial_action, discounts_accumulate_reward, end_state, self.args.gamma, end_terminal, end_done)  # Store the transition
            ...
            if done:
                #store rest of data remaining in replay buffer
...
if __name__ == '__main__':
    ...
    parser.add_argument("--N_steps", type=int, default=3, help="N_steps return")
